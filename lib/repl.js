...
const writer = w => output => {
    if (util.isError(output)) return output.message;
        ...
        if (!output.description.includes(symbols.pass)) output.description = symbols.pass + output.description;
    ...
    for (let func in target) {
        if (target[func].constructor.name === 'AsyncFunction')
            ...
            repl.context[func] = function () {
                ...
                if (res.exists) {
                    let logExists = async () => {
                        let msg = symbols.pass + 'Does not Exist';
                        if (await res.exists()) msg = symbols.pass + 'Exists';
                        console.log(removeQuotes(util.inspect(msg, { colors: true }), msg));
                    };
                    logExists();
                }
                return res;
